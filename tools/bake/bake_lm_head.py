...
Run INSIDE the vllm image container (torch needed), from the repo root:
  docker run --rm -v "$PWD":/repo -v /path/to/calib:/calib -v /path/to/out:/out \
    -v ~/.cache/huggingface/hub/models--nvidia--Qwen3.8-Flash-Next-NVFP4/snapshots/<sha>:/src:ro \
    --entrypoint python3 vllm/vllm-openai:qwen38-flash-next /repo/tools/bake/bake_lm_head.py \
    --snapshot /src --hs-dir /calib/hs --token-map /repo/tools/bake/marker_token_map.json \
    --lam 2.0 --out /out/baked_model
